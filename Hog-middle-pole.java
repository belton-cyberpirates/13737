...
public class AutoLeft13737 extends LinearOpMode {
  ...
  public void runOpMode() {
    ...
    if (opModeIsActive()) { // <----------------------------------------------------------------
     ...
     driveMotors.turn(90);
     DriveMotors.move(direction.TILE_LENGTH);
     DriveMotors.turn(-90);
     DriveMotors.move(Direction.FORWARD),(int)(Config.TILE_LENGTH*4);
     DriveMotors.Turn(-45);
     // mediun pole
     arm.Move(config.MID_POLE_HEIGHT)
     DriveMotors.move(forward.BUMP),true;
     // should go down low but we do not have a function for that
     arm.Move(config.LOW_POLE_HEIGHT);
     claw.open();
     DriveMotors.move(direction.BACKWARD.BUMP);;
     DriveMotors.turn(45);
     driveMotors.move(direction.BACKWARD);
     DriveMotors.turn(-90);
